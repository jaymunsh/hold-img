...
[Releases](https://github.com/jaymunsh/hold-img/releases)에서 `HoldImg.app.zip`을 받아 압축을 풀고 `/Applications`에 넣으세요.
...
> 모든 처리는 Mac 안에서만 이뤄지며 외부로 전송되는 데이터는 없습니다.
...
펜 모드(`P`)에서는 주석 도구를 제공합니다 — **펜(자유곡선) · 형광펜(반투명 강조 밴드, 드래그 중 되돌아가면 지워짐) · 화살표 ·
사각형 · 모자이크 · 텍스트(`⇧Enter` 줄바꿈, `⌘+`/`⌘-` 크기 조절) · 이동(손)**, 색상 4색, 되돌리기(`⌘Z`), 전체 지우기.
모자이크는 민감정보를 가릴 때 유용하고, 이동 도구로는 그려둔 주석을 잡아 옮길 수 있습니다.
완료(✓)하면 주석이 원본 해상도로 이미지에 굽혀서
...
UI는 **한국어와 영어**를 지원합니다. 설정 → 언어에서
시스템 기본값 · 한국어 · English 중 선택할 수 있습니다.

## 버전 로그

전체 업데이트 내역은 [CHANGELOG.md](CHANGELOG.md) 또는
[GitHub Releases](https://github.com/jaymunsh/hold-img/releases)에서
확인할 수 있습니다 — HoldImg 설정 창 최하단에도 같은 링크가 있습니다.

## 라이선스

MIT — [LICENSE](LICENSE) 참고.
[KeyboardShortcuts](https://github.com/sindresorhus/KeyboardShortcuts)(MIT)를 사용합니다.
