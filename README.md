...
- **영역 캡처** `⌃⌥C` — 화면이 프리즈되고 드래그로 영역 선택 → 즉시 플로팅
  - 비율 고정: `1` 자유 · `2` 1:1 · `3` 4:3 · `4` 16:9 · `5` 16:10
...
- 캡처 화면에서 돋보기(`M` 토글) + `C`로 픽셀 HEX 컬러 복사

## 플로팅 창 조작

| 동작 | 방법 |
|---|---|
| 이동 | 드래그 (화면/다른 창 모서리·중앙에 자동 스냅) |
| 파일로 드래그 | 우클릭 드래그 — Finder/슬랙/메일 등에 이미지로 드롭 |
| 크기 조절 | 스크롤 (커서 기준 줌), 모서리 드래그 — 기본 비율 유지, 툴바 🔒 해제 시 자유 |
| 축소/복원 | 더블클릭 — 썸네일로 접기/펴기 |
| 이미지 복사 | 클릭 또는 `⌘C` |
| 저장 | `⌘S` (PNG/JPEG) |
| 펜으로 표시 | `P` — 이미지 위에 자유곡선, 완료 시 이미지에 적용 |
| 항상 위 토글 | `T` 또는 우클릭 메뉴 |
...
저장 · 닫기 버튼. 펜 모드에서는 색상(4색) · 되돌리기(`⌘Z`) ·
전체 지우기 · 완료를 제공하며, 완료하면 스트로크가 이미지에 굽혀서
복사/저장에 그대로 반영됩니다.
