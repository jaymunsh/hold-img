...
if security find-certificate -c "HoldImg Local Dev" "$KC" >/dev/null 2>&1; then
  # Unlock the dev keychain so codesign doesn't block on a prompt.
  # The password lives in an env var or an untracked local file — never in this repo.
  KPW="${HOLDIMG_KEYCHAIN_PASSWORD:-$(cat "$HOME/.local/share/holdimg-dev/keychain-password" 2>/dev/null)}"
  if [ -n "$KPW" ]; then
    security unlock-keychain -p "$KPW" "$KC" 2>/dev/null || true
  fi
