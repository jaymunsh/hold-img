...
if security find-certificate -c "HoldImg Local Dev" "$KC" >/dev/null 2>&1; then
  # The dev keychain can be locked between sessions; unlock so codesign
  # does not block waiting for a password prompt.
  security unlock-keychain -p holdimg "$KC" 2>/dev/null || true
